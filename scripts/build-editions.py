...
<head>
<meta charset="UTF-8">
<!-- Link corto con rastreo intacto: ?w=s (sala) / ?w=c (compartir) se expande a utm_* ANTES de que GA mire -->
<script>(function(){try{var q=new URLSearchParams(location.search),w=q.get('w');if(!w||q.get('utm_source'))return;var med={s:'sala',c:'compartir',e:'lunes'}[w];if(!med)return;var p=location.pathname.split('/').filter(Boolean);var slug=p[p.length-1]==='en'?p[p.length-2]:p[p.length-1];q.delete('w');q.set('utm_source',w==='e'?'email':'whatsapp');q.set('utm_medium',med);q.set('utm_campaign',slug||'');history.replaceState(null,'',location.pathname+'?'+q.toString()+location.hash);}catch(e){}})();</script>
<!-- GA4: el vinilo de las ediciones reporta a la misma casa que la app -->
...
.btn-ig-icon path { stroke: #1A1A1A; }
.btn-row-marca { grid-template-columns: 1fr 48px 48px; }
...
@keyframes vinAliento{0%,100%{opacity:.32;transform:scale(.96)}50%{opacity:.6;transform:scale(1.04)}}
#vinLuz{position:absolute;inset:0;border-radius:inherit;pointer-events:none;z-index:1;background:linear-gradient(115deg,transparent 42%,rgba(255,255,255,.5) 50%,transparent 58%);background-size:280% 100%;background-position:160% 0;animation:vinBarrido 5.8s ease-in-out infinite}
@keyframes vinBarrido{0%,72%{background-position:160% 0}88%{background-position:-60% 0}100%{background-position:-60% 0}}
#vinilo.abierta #vinLuz{animation:vinDestello .8s cubic-bezier(.19,1,.22,1) both}
@keyframes vinDestello{from{background-position:160% 0}to{background-position:-60% 0}}
...
#viniloPie{display:none;font:italic 500 10.5px/1.45 'Poppins',sans-serif;color:rgba(255,255,255,.74);white-space:normal;margin-top:3px;text-shadow:0 1px 3px rgba(0,0,0,.35)}
...
#vinBorde{position:absolute;inset:0;border-radius:inherit;padding:1.2px;background:linear-gradient(160deg,rgba(255,255,255,.6),rgba(255,255,255,.14) 32%,rgba(255,255,255,.05) 62%,color-mix(in srgb,var(--accent,#fff) 45%,transparent));-webkit-mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);-webkit-mask-composite:xor;mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:exclude;opacity:0;transition:opacity .3s .12s;pointer-events:none;z-index:2}
#vinilo.abierta #vinBorde{opacity:1}
#vinilo.abierta #vinBar{opacity:.95}
...
<div id="vinilo" aria-hidden="true" role="group" aria-label="Música de esta edición">
  <div id="vinAro"></div>
  ...
  <div class="reveal-card" onclick="event.stopPropagation()">
    ...
    <div class="card-inner">
      ...
      <div class="card-actions">
        <div class="btn-row btn-row-2 btn-row-marca">
          ...
          <a class="btn-light btn-wa" id="btnWa" href="#" target="_blank" rel="noopener noreferrer" aria-label="Compartir por WhatsApp" title="Compartir por WhatsApp">
            <svg class="btn-ig-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              <path d="M12 3.5v11" stroke="white" stroke-width="1.8" stroke-linecap="round"/>
              <path d="M8.2 7.2 12 3.4l3.8 3.8" stroke="white" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
              <path d="M6.5 10.5H6a2 2 0 0 0-2 2V18a2.5 2.5 0 0 0 2.5 2.5h11A2.5 2.5 0 0 0 20 18v-5.5a2 2 0 0 0-2-2h-.5" stroke="white" stroke-width="1.8" stroke-linecap="round"/>
            </svg>
          </a>
        </div>
...
/* 🎵 vinilo v4 — morfosis. Material clonado en runtime del primer botón cta (herencia literal).
   Estados: reposo → (tap = play+abierta) → 4s → sonando colapsado con anillo → 28s despedida → rotación.
   Badge bilingüe sigue a window.tgLang. Reveal (portada/cta visibles) => fuera; la música continúa. */
(function(){
  ...
  function badgeLang(){ if(!bdg||!bdg.src) return;
    ...
    if(bdg.src.indexOf('/'+l)<0) bdg.src='https://tools.applemediaservices.com/api/badges/listen-on-apple-music/badge/'+l; }
  function heredaMaterial(intento){ intento=intento||0;
    try{ var b=document.querySelector('.tg-cta button');
  ...
  function icon(p){ icoP.style.display=p?'none':'block'; icoS.style.display=p?'block':'none';
    el.classList.toggle('sonando',p); nuc.setAttribute('aria-label',p?'Pausa':'Reproducir 30 segundos'); }
  function abre(){ el.classList.add('abierta'); reagenda(); }
  function cierra(){ el.classList.remove('abierta'); }
  ...
  function play(){ var c=cola[i]; if(fadeT){clearInterval(fadeT);fadeT=null;} au.volume=1;
    ...
    au.play().then(function(){ icon(true); fallos=0;
      ...
      try{ if('mediaSession' in navigator){
        ...
        navigator.mediaSession.setActionHandler('play',function(){play();}); } }catch(e){}
    }).catch(function(){ icon(false); }); }
  function onda(){ el.classList.remove('toco'); void el.offsetWidth; el.classList.add('toco'); }
  nuc.addEventListener('click',function(ev){ ev.stopPropagation(); onda();
    if(!el.classList.contains('abierta')){ abre(); if(au.paused) play(); return; }
    if(au.paused){ play(); } else { apaga(false); } reagenda(); });
  el.addEventListener('click',function(){ if(!el.classList.contains('abierta')){ abre(); if(au.paused) play(); } else { reagenda(); } });
  var pill=document.getElementById('tgLangPill');
  if(pill) pill.addEventListener('click',function(){ setTimeout(badgeLang,60); });
  pinta();
  ...
  function entra(){ heredaMaterial(); el.classList.add('on','nace');
    ...
    setTimeout(onda,430);
    el.removeAttribute('aria-hidden'); if(!autoP) vigia(); }
  var autoP=(function(){ try{ return new URLSearchParams(location.search).get('p')==='1'; }catch(e){ return false; } })();   // 🎧 llegó desde el mensaje de la pieza: el vinilo entra ya, sin esperar overlay ni esconderse
  ...
  if(autoP){ intenta(); } else if(ov){ if(ov.classList.contains('visible')) visto=true;
    new MutationObserver(function(){ var v=ov.classList.contains('visible');
      if(v){ visto=true; } else if(visto){ setTimeout(intenta,250); } })
      .observe(ov,{attributes:true,attributeFilter:['class']});
    setTimeout(function(){ if(!visto) intenta(); },9000);
  } else { intenta(); }
})();
...
<script id="wa-share-js">
/* 🟢 Compartir por WhatsApp: frase + link marcado (utm) en el idioma de la página. El lector elige a quién. */
(function(){try{var a=document.getElementById('btnWa');if(!a||typeof state==='undefined')return;
var en=(typeof tgLang!=='undefined'&&tgLang==='en');
var fr=((en&&state.frases_en&&state.frases_en[0])||(state.frases&&state.frases[0])||'').trim();
var url=location.origin+location.pathname+'?w=c';
var txt=(fr?fr+'\\n':'')+url,movil=/iPhone|iPad|Android/i.test(navigator.userAgent);a.href=movil?('https://wa.me/?text='+encodeURIComponent(txt)):('https://web.whatsapp.com/send?text='+encodeURIComponent(txt));if(movil&&navigator.share){a.addEventListener('click',function(ev){ev.preventDefault();navigator.share({text:txt}).catch(function(){location.href='https://wa.me/?text='+encodeURIComponent(txt);});});}
a.title=en?'Share on WhatsApp':'Compartir por WhatsApp';a.setAttribute('aria-label',a.title);
a.addEventListener('click',function(){try{gtag('event','share_wa',{contexto:'edicion',slug:state.id||'',idioma:en?'en':'es'});}catch(e){}});
}catch(e){}})();
</script>
<script id="vin-auto-js">
/* 🎧 ?p=1 (viene del mensaje de la pieza): al montar el vinilo, se abre la barra y se intenta tocar; si el navegador exige un toque, la barra queda abierta esperándolo. */
(function(){try{if(new URLSearchParams(location.search).get('p')!=='1')return;var hecho=false;function arranque(){if(hecho)return;var b=document.getElementById('vinNucleo'),v=document.getElementById('vinilo');if(!b||!v||!v.classList.contains('on'))return;hecho=true;document.removeEventListener('click',arranque,true);if(!v.classList.contains('abierta'))b.click();}document.addEventListener('click',arranque,true);}catch(e){}})();
</script>
</body>
</html>
"""
