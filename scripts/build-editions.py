...
<script>(function(){try{var q=new URLSearchParams(location.search),w=q.get('w');if(!w||q.get('utm_source'))return;var med={s:'sala',c:'compartir'}[w];if(!med)return;var p=location.pathname.split('/').filter(Boolean);var slug=p[p.length-1]==='en'?p[p.length-2]:p[p.length-1];q.delete('w');q.set('utm_source','whatsapp');q.set('utm_medium',med);q.set('utm_campaign',slug||'');history.replaceState(null,'',location.pathname+'?'+q.toString()+location.hash);}catch(e){}})();</script>
