...
/* 🎵 vinilo v3 — barra anclada estilo now-playing (carátula + badge oficial; jamás isla-anuncio) */
#vinilo{position:fixed;left:0;right:0;bottom:0;z-index:700;display:none;align-items:center;gap:12px;padding:10px 14px calc(10px + env(safe-area-inset-bottom,0px));border-radius:16px 16px 0 0;background:rgba(12,12,14,.62);-webkit-backdrop-filter:blur(20px) saturate(1.3);backdrop-filter:blur(20px) saturate(1.3);border-top:1px solid rgba(255,255,255,.08);box-shadow:0 -8px 30px rgba(0,0,0,.35);overflow:hidden}
#vinilo.on{display:flex;animation:vinSube .55s cubic-bezier(.19,1,.22,1)}
@keyframes vinSube{from{transform:translateY(110%)}to{transform:none}}
#vinilo.fuera{display:none !important}
#viniloCov{flex:0 0 42px;width:42px;height:42px;border-radius:9px;object-fit:cover;background:rgba(255,255,255,.06);box-shadow:0 2px 10px rgba(0,0,0,.45)}
#viniloBtn{flex:0 0 34px;width:34px;height:34px;border-radius:50%;border:0;background:var(--accent,#fff);color:#0b0b0d;display:flex;align-items:center;justify-content:center;cursor:pointer;-webkit-tap-highlight-color:transparent}
#viniloBtn:active{transform:scale(.95)}
#viniloBtn svg{width:14px;height:14px;display:block}
...
#viniloLink{flex:0 0 auto;display:flex;align-items:center;text-decoration:none}
#viniloLink img{height:22px;display:block;opacity:.92}
#viniloLink:active img{opacity:1}
#vinBar{position:absolute;left:0;top:0;height:2px;width:0%;background:var(--accent,#fff);opacity:.95;transition:width .25s linear}
...
<div id="vinilo" aria-hidden="true">
  ...
  <a id="viniloLink" href="#" target="_blank" rel="noopener noreferrer" aria-label="Escúchala completa en Apple Music"><img src="https://tools.applemediaservices.com/api/badges/listen-on-apple-music/badge/es-mx" alt="Apple Music" onerror="this.parentNode.textContent='Apple Music ↗';this.parentNode.style.font='600 11px Poppins';this.parentNode.style.color='rgba(255,255,255,.6)'" /></a>
  <div id="vinBar"></div>
</div>
...
(function(){
  ...
  var el=document.getElementById('vinilo'), btn=document.getElementById('viniloBtn'),
      ...
      lnk=document.getElementById('viniloLink'), bar=document.getElementById('vinBar'), cov=document.getElementById('viniloCov'),
  ...
  function pinta(){ var c=cola[i]; tit.textContent=c.cancion; art.textContent=c.artista;
    if(c.link) lnk.href=c.link; if(c.art){ cov.src=c.art; cov.alt=c.album||c.cancion; } bar.style.width='0%'; }
