...
#tgStreak{position:fixed;top:max(54px, calc(env(safe-area-inset-top,0px) + 14px));left:16px;z-index:650;display:none;align-items:center;background:rgba(255,255,255,.05);border:.5px solid rgba(255,255,255,.1);border-radius:99px;padding:5px 11px;font:600 11px/1 'Poppins',sans-serif;color:#fff}
#tgStreak.visible{display:flex}
#tgStreak .bm-flame{font-size:13px;filter:drop-shadow(0 0 4px rgba(255,94,0,.6));margin-right:3px}
#tgStreak #tgStreakNum{font-weight:700;color:#fff;margin-right:3px}
#tgStreak #tgStreakUnit{font-weight:400;color:rgba(255,255,255,.55)}
...
<div id="tgStreak" aria-hidden="true"><span class="bm-flame">\U0001F525</span><span id="tgStreakNum">0</span><span id="tgStreakUnit">d\u00edas</span></div>
...
function tgSetLang(l){ if(l===tgLang){ tgPillPaint(); return; } tgLang=l; try{ localStorage.setItem('triggui_lang', JSON.stringify(l)); }catch(e){} tgApplyGrid(); tgRepaintGrid(); tgApplyCard(); tgApplySilence(); tgPillPaint(); tgPaintStreak(); }
tgApplyGrid(); /* swap pre-pintado: el grid nace en el idioma correcto */
/* \U0001F525 racha homologada: MISMAS llaves que la app (triggui_streak / triggui_last_visit) \u2014 abrir una edici\u00f3n cuenta igual que abrir la app */
function tgGetL(k,d){ try{ var v=localStorage.getItem('triggui_'+k); return v===null?d:JSON.parse(v);}catch(e){return d;} }
function tgSetL(k,v){ try{ localStorage.setItem('triggui_'+k, JSON.stringify(v)); }catch(e){} }
function tgUpdateStreak(){ var today=new Date().toDateString(); var last=tgGetL('last_visit',null); var st=tgGetL('streak',0);
  if(st===0){ tgSetL('streak',1); tgSetL('last_visit',today); return 1; }
  if(last===today) return st;
  var y=new Date(); y.setDate(y.getDate()-1);
  var ns=(last===y.toDateString())? st+1 : 1;
  tgSetL('streak',ns); tgSetL('last_visit',today);
  try{ gtag('event','streak_updated',{streak:ns}); }catch(e){}
  return ns; }
function tgPaintStreak(){ var el=document.getElementById('tgStreak'); if(!el) return; var n=tgUpdateStreak();
  document.getElementById('tgStreakNum').textContent=n;
  document.getElementById('tgStreakUnit').textContent=(tgLang==='en')?((n===1)?'day':'days'):((n===1)?'d\u00eda':'d\u00edas');
  if(n>0) el.classList.add('visible'); }
document.addEventListener('DOMContentLoaded', function(){ tgPaintStreak(); tgApplyCard(); tgApplySilence(); tgPillPaint(); const a=document.getElementById('tgLangEs'), b=document.getElementById('tgLangEn'); if(a)a.addEventListener('click',function(){tgSetLang('es');}); if(b)b.addEventListener('click',function(){tgSetLang('en');}); const pl=document.getElementById('tgLangPill'); if(pl)pl.addEventListener('click',function(ev){ if(ev.target&&ev.target.closest&&ev.target.closest('#tgLangEs,#tgLangEn'))return; tgSetLang(tgLang==='en'?'es':'en'); }); const _ro=document.getElementById('revealOverlay'); if(_ro&&pl){ const _sync=function(){ const cs=getComputedStyle(_ro); const abierto=(cs.display!=='none'); pl.style.display=abierto?'none':'flex'; var st=document.getElementById('tgStreak'); if(st) st.style.display=abierto?'none':(st.classList.contains('visible')?'flex':'none'); }; new MutationObserver(_sync).observe(_ro,{attributes:true,attributeFilter:['class','style']}); _sync(); } });
