...
#vinilo.sonando:not(.abierta) #vinAro{opacity:1}
...
#vinNucleo svg{width:17px;height:17px;display:block;filter:drop-shadow(0 1px 2px rgba(0,0,0,.4))}
...
#vinBar{position:absolute;left:0;top:0;height:2px;width:var(--vin-p,0%);background:var(--accent,#fff);opacity:0;transition:width .25s linear,opacity .2s;z-index:3}
...
(function(){
  ...
  au.addEventListener('timeupdate',function(){ var t=au.currentTime;
    el.style.setProperty('--vin-p',Math.min(100,(t/30)*100)+'%');
    if(t>=28) despedida(); });
  ...
  nuc.addEventListener('click',function(ev){ ev.stopPropagation();
