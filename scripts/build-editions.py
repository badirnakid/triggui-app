...
function armarCuerpo(){
  ...
  function __gemelo(t){
    var n = String(t || '');
    var largo = Math.max(__poolES.length, __poolEN.length);
    for (var gi = 0; gi < largo; gi++) {
      var es = __txtP(__poolES[gi]), en = __txtP(__poolEN[gi]);
      if (es === n || en === n) return { es: es || n, en: en || '' };
    }
    return { es: n, en: '' };
  }
