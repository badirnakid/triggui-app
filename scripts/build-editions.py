...
.tg-aur{position:absolute;width:52vmax;height:52vmax;border-radius:50%;filter:blur(70px);opacity:.55;pointer-events:none;will-change:transform;animation:tgDrift 22s ease-in-out infinite alternate}
.tg-aur-b{top:auto;left:auto;bottom:-14vmax;right:-14vmax;animation-duration:27s;animation-direction:alternate-reverse}
.tg-aur-a{top:-14vmax;left:-14vmax}
.tg-aur-c{top:30%;left:30%;width:44vmax;height:44vmax;animation-duration:31s}
@keyframes tgDrift{0%{transform:translate(-4%,-2%) scale(1)}100%{transform:translate(6%,5%) scale(1.15)}}
...
(function(){
  function hexA(hex, a){
    ...
    if (!m) return 'rgba(40,100,220,' + a + ')';
    var n = parseInt(m[1], 16);
    return 'rgba(' + ((n >> 16) & 255) + ',' + ((n >> 8) & 255) + ',' + (n & 255) + ',' + a + ')';
  }
  var cs = (st.colores || []);
  var defs = [[cs[1] || cs[0], 0.45, 'a'], [cs[2] || cs[3], 0.32, 'b'], [cs[0] || cs[2], 0.24, 'c']];
  for (var i = 0; i < 3; i++) {
    var d = document.createElement('div');
    d.className = 'tg-aur tg-aur-' + defs[i][2];
    d.style.background = hexA(defs[i][0], defs[i][1]);
    capa.insertBefore(d, capa.firstChild);
  }
})();
...
function render(){
  ...
  for (var i = 0; i < 4; i++) (function(i){
    ...
    if (i === portIdx) {
      ...
      card.onclick = function(e){
        if (revealed) { e.stopPropagation(); abrirTarjeta(); }
        else card.classList.toggle('show');
      };
    }
