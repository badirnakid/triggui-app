...
.silence-screen .sil-sub {{
  ...
  line-height: 1.4; color: rgba(255,255,255,.45);
...
.silence-screen .sil-mark {{
  opacity: 0.2;
  animation: silCallIn 1s ease 1s both;
}}
...
function setOverlayView(next) {{
  ...
  const visible = next !== 'blocks';
  revealOverlay.style.opacity = visible ? '1' : '0';
  revealOverlay.style.pointerEvents = visible ? 'auto' : 'none';
...
function renderBlocks() {{
  grid.innerHTML = chronoOrder.map((realIdx, idx) => `
    <div class="block" data-idx="${{idx}}" data-real-idx="${{realIdx}}" style="background:${{grad(realIdx)}}">
      <span class="block-hit" aria-hidden="true"></span>
...
function handleBlockActivate(e) {{
  const hit = e.target.closest('.block-hit, .block');
  if (!hit) return;
...
grid.addEventListener('pointerup', handleBlockActivate);
