...
.btn-icon {{
  ...
  background: #FFFFFF; /* Botones blancos clínicos */
...
.btn-icon .btn-triggui-logo {{ height: 20px; filter: none; }} /* Logo negro nativo */
.btn-icon .btn-ig-icon {{ width: 18px; height: 18px; }}
/* Adaptación de SVG de Instagram para modo claro */
.btn-ig-icon rect, .btn-ig-icon circle {{ stroke: #1A1A1A; }}
.btn-ig-icon circle[fill="white"] {{ fill: #1A1A1A; }}

.btn-icon.btn-penguin-icon img {{ height: 36px; filter: grayscale(1) brightness(0.2); }}
...
/* === PANTALLA DE SILENCIO (Se mantiene cinematográfica y oscura) === */
.silence-screen {{
