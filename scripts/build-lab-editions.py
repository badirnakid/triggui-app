...
/* === MAGIA CINEMATOGRAFICA: PANTALLA DE SILENCIO === */
.silence-screen {{
  display: none;
  position: absolute;
  inset: 0;
...
.silence-screen::before {{
  ...
  background: radial-gradient(circle, {esc(t_accent)}35 0%, transparent 70%);
  filter: blur(40px);
  z-index: 0;
  pointer-events: none;
...
.silence-screen .sil-cover {{
  ...
  box-shadow: 0 30px 60px -10px rgba(0,0,0,0.9), 0 0 50px {esc(t_accent)}40;
  margin-bottom: 40px;
  cursor: pointer;
  animation: floatMagic 5s ease-in-out infinite;
}}
@keyframes floatMagic {{
  0%, 100% {{ transform: translateY(0); box-shadow: 0 30px 60px -10px rgba(0,0,0,0.9), 0 0 50px {esc(t_accent)}40; }}
  50% {{ transform: translateY(-12px); box-shadow: 0 45px 75px -10px rgba(0,0,0,0.7), 0 0 70px {esc(t_accent)}65; }}
}}
