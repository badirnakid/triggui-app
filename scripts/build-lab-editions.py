...
.reveal-card {{
  ...
  overflow: visible; /* AQUI ESTA EL EXORCISMO PARA EL PADRE */
  cursor: default;
...
.ed-cover-wrap {{
  ...
  position: relative;
  touch-action: manipulation;
  /* Le damos un padcito para que respire sin chocar */
  padding: 8px; 
}}
.ed-cover-wrap img {{
  display: block;
...
/* === MAGIA NIVEL DIOS PARA LA PANTALLA DE SILENCIO === */
.silence-screen {{
  display: none;
  position: absolute;
  inset: 0;
...
.silence-screen::before {{
  ...
  background: radial-gradient(circle, {esc(t_accent)}30 0%, transparent 65%);
  filter: blur(35px);
  z-index: 0;
  pointer-events: none;
...
.silence-screen .sil-cover {{
  ...
  height: auto;
  border-radius: 12px;
  opacity: 1; /* 100% brillo de tarjeta de poder */
  border: 1px solid rgba(255,255,255,0.15);
  box-shadow: 0 30px 60px -10px rgba(0,0,0,0.9), 0 0 40px {esc(t_accent)}40;
  margin-bottom: 40px;
  cursor: pointer;
  animation: floatMagic 5s ease-in-out infinite; /* Levitación */
}}
@keyframes floatMagic {{
  0%, 100% {{ transform: translateY(0); box-shadow: 0 30px 60px -10px rgba(0,0,0,0.9), 0 0 40px {esc(t_accent)}40; }}
  50% {{ transform: translateY(-12px); box-shadow: 0 40px 70px -10px rgba(0,0,0,0.7), 0 0 60px {esc(t_accent)}60; }}
}}
...
@media (max-width: 640px) {{
  ...
  .silence-screen .sil-cover {{ width: 150px; }}
}}
</style>
</head>
