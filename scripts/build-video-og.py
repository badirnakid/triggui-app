...
async def render(libro, out_path):
    v = next((x for x in ((libro.get("_video") or {}).get("candidatos") or []) if x.get("id")), None)
    if not v: return False
    col = libro.get("colores") or []; a = col[0] if col else "#E8A838"; b = col[1] if len(col) > 1 else a
    t = open("scripts/templates/og-video.html", encoding="utf-8").read(); tit = v.get("titulo", "")
    t = (t.replace("{{ACCENT}}", esc(a)).replace("{{ACCENT2}}", esc(b)).replace("{{THUMB}}", thumb(v["id"]))
