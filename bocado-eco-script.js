// ═══════════════════════════════════════════════════════════════════════
// 🌒 TRIGGUI BOCADO Y ECO v7 — Diagnóstico cuántico del ciclo completo
// ═══════════════════════════════════════════════════════════════════════
//
// PROBLEMAS REPORTADOS EN V6:
//
// BUG D — "Después del bocado, se ve rápidamente la barra mágica y luego
//          luego entran los bloques" (todas las vueltas):
//   Mi v6 quitaba bm-content-fading al final del bocado para arreglar
//   BUG B (botones primera vuelta atmósfera vacía). Pero eso hacía visible
//   el inner ANTES de que el overlay iniciara fade-out (650ms). Flash.
//
//   FIX v7: NO quitar fading al final del bocado. En su lugar, limpiar
//   fading dentro de openOverlay y applyStrings (con guardas para no
//   romper bocado/eco activos).
//
// BUG E — Segunda vuelta: primer click "selecciona", segundo "reinicia con
//         eco" en lugar de abrir tarjeta:
//   Mi showEco aplicaba inline tarjeta.style.opacity='0' + display='none'.
//   Al final del eco, v6 solo restauraba transition. opacity y display
//   QUEDABAN como inline styles persistentes.
//
//   Cuando la segunda vuelta abría tarjeta:
//     - btnShare.onclick: tarjeta.style.display='flex' → sobrescribe inline ✓
//     - classList.add('visible') → CSS .visible dice opacity:1 PERO inline
//       opacity:'0' GANA sobre clase con !important.
//   Tarjeta quedaba display:flex + opacity:0 (invisible pero clickable).
//   Click en bloque → btnShare.click() → tarjeta abre invisible.
//   Segundo click → cae sobre tarjetaOverlay invisible (z-index 999) →
//   onclick="closeCard()" → showEco. ¡Eco en lugar de tarjeta visible!
//
//   FIX v7: al FINAL del eco, limpiar TODOS los inline styles del
//   tarjetaOverlay (opacity, display, transition). El CSS por default
//   aplica display:none + opacity:0 cuando no tiene clase 'visible'.
//
// FIXES v7 mantiene:
//   - Override CSS nuclear (mata animación CSS nativa de bloques)
//   - Sin inline styles en bloques (solo classes)
//   - Defensa anti doble-trigger lupa
// ═══════════════════════════════════════════════════════════════════════

(function setupBocadoEcoV7() {
  'use strict';
  ...
  function ensureStyles() {
    ...
    style.textContent = [
      ...
      '}',
      // ═══════════════════════════════════════════════════════════════════
      // 🔥 OVERRIDE NUCLEAR: matar animación CSS nativa de bloques
      // ═══════════════════════════════════════════════════════════════════
  ...
  // 🔥 v7: Limpia bm-content-fading del inner del overlay barra mágica.
  // Llamada por openOverlay y applyStrings (con guardas) ANTES de mostrar
  // para que el contenido no aparezca invisible cuando la barra mágica
  // se reabre después de un bocado/eco.
  // NO toca bm-content-restoring (es transitorio, lo controla el eco).
  ...
  function showBocado() {
    ...
    // 🔥 FIX v7 BUG D: NO quitar fading aquí. El inner debe quedar invisible
    // hasta que el overlay se cierre. cleanupContentFading se llamará desde
    // openOverlay/applyStrings cuando el overlay se reabra después.
    setTimeout(function() {
    ...
    if (typeof console !== 'undefined' && console.log) console.log('[Triggui Bocado v7]', '"' + phrase.text + '"', '←', phrase.source);
  }

  // ── BLOQUES (sin inline styles) ──
  ...
  function showEco() {
    ...
    // 🔥 FIX v7 BUG E: ya NO guardamos inline styles para "restaurar" — los
    // limpiaremos completamente al final. Esto es porque cualquier inline
    // style residual rompe la próxima apertura de tarjeta.
    if (tarjeta) {
    ...
    if (!overlay) {
      ecoActive = false;
      // 🔥 FIX v7: limpiar inline styles del tarjetaOverlay aún si overlay no existe
      if (tarjeta) {
    ...
    requestAnimationFrame(function() {
      ...
      setTimeout(function() {
        ...
        // 🔥 FIX v7 BUG E: limpiar TODOS los inline styles del tarjetaOverlay
        // para que CSS default aplique (display:none + opacity:0). Cuando la
        // tarjeta se vuelva a abrir, los nuevos inline styles del btnShare
        // se aplicarán limpiamente sin conflictos persistentes.
        if (tarjeta) {
          tarjeta.style.transition = '';
          tarjeta.style.opacity = '';
          tarjeta.style.display = '';
          tarjeta.classList.remove('visible'); // por si acaso quedó
        }
    ...
    if (typeof console !== 'undefined' && console.log) console.log('[Triggui Eco v7]', '"' + phrase.text + '"', '←', phrase.source);
  }

  function patchBarraMagica() {
    if (typeof BarraMagica === 'undefined') return false;
    if (BarraMagica._bocadoEcoV7Patched) return true;
    ...
    // 🔥 FIX v7 BUG D: parchear applyStrings para limpiar fading residual
    // antes de aplicar strings. Guarda: si bocado o eco están activos, no
    // limpiar (ellos manejan su propio fading).
    ...
    BarraMagica._bocadoEcoV7Patched = true;
    return true;
  }

  function patchCloseCard() {
    if (typeof window.closeCard !== 'function') return false;
    if (window._closeCardBocadoEcoV7Patched) return true;
    ...
    window._closeCardBocadoEcoV7Patched = true;
    return true;
  }

  function patchOpenOverlayForLupa() {
    if (typeof BarraMagica === 'undefined') return false;
    if (BarraMagica._lupaBocadoEcoV7Patched) return true;
    ...
    BarraMagica.openOverlay = function(force) {
      ...
      // 🔥 FIX v7 BUG D: limpiar fading residual del bocado/eco previo
      // ANTES de mostrar overlay. Defensa adicional al patch de applyStrings.
      cleanupContentFading();

      return original(force);
    };

    BarraMagica._lupaBocadoEcoV7Patched = true;
    return true;
  }
  ...
  function tryInstall() {
    ...
    if (typeof console !== 'undefined' && console.log) {
      console.log('[Triggui BocadoEco v7] ' +
        (ok1 && ok2 && ok3 ? 'instalado nivel quark — fixes D E activos' : 'parcial') +
